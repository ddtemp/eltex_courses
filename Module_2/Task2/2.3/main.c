...
int main() {
    void *calc_lib = dlopen("./libcalc.so", RTLD_LAZY); // RTLD_LAZY или RTLD_NOW, в зависимости от вашего выбора
    if (!calc_lib) {
    ...
    double (*exponentiation)(double, double) = dlsym(calc_lib, "exponentiation"); // Добавляем указатель на функцию exponentiation

    if (!addition || !subtraction || !multiplication || !division || !exponentiation) { // Добавляем проверку на указатель exponentiation
    ...
    while (1) {
        ...
        if (fgets(input, sizeof(input), stdin) != NULL) {
            ...
            switch (choice) {
                ...
                case 5:
                    result = exponentiation(num_1, num_2); // Используем функцию exponentiation
                    break;
